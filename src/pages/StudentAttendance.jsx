...
const StudentAttendance = ({ clsOp }) => {
  ...
  const getStudents = async () =>{
    try {
      // let response = await axios.get(`http://localhost:3501/api/v1/${newInpu.dptName.id}/all-students?currentYear=${newInpu.year.id}&degree=${newInpu.dgree}&field=name&sort=name`)
      let response = await getAllStudents(newInpu.dptName.id,`currentYear=${newInpu.year.id}&degree=${newInpu.dgree}&field=name&sort=name`)
  ...
  const departmentNames = [{id:"CS",name:"Computer science"},{id:"BCOM",name:"B.COM"},{id:"BCA",name:"BCA"}]
  ...
  function succssNotification (msg){
    toast.success(msg,{
      style: {
        width: '300px', 
        height: '10px',
  ...
  const handlePostAttendance = async () =>{
    ...
    try {
      // let response = await axios.post(`http://localhost:3501/api/v1/${newInpu.dptName.id}/attendance`,devData)
  ...
  const handleOnSubmit = (e) =>{
    ...
    if(success){
      succssNotification("Attendance created successfully")
      setPutAddnese(true)
      console.log(newInpu)
    }
  ...
  return (
    <>
    {!putAddnese && <main className=" h-[100vh]  w-[100%]  flex flex-col justify-center  ">

      <h1 className=" text-center text-3xl font-mono  w-[90%] mx-auto">CREATE ATTENDANCE</h1>
      <form onSubmit={handleOnSubmit} className=" w-[90%] mt-5 h-[50%] lg:w-[60%]  relative  mx-auto" >
    ...
    {/* display attendse deatails for given above input */}
   ...
   {putAddnese && <section className=" h-[150vh] relative ">
      ...
      <ul className=" text-[12px] lg:text-[20px] p-1 w-4/5 sm:w-4/6">
          <li className="flex justify-evenly px-3 bg-gray-100   py-2 shadow-md rounded ">
            ...
            <p className=" w-1/5 text-center font-bold font-mono  ">Addnese</p>
          </li>
        </ul>
