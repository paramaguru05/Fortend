...
const StudentLogin = () =>{
  ...
  const handleOnSubmit = async () =>{
    ...
    if( success ){
      ...
      input.DOB = dob.join("-")
      try {
        // let response = await axios.post(`http://localhost:3501/api/v1/${input.dptName.value}/studentLogin`,{id:input.id,DOB:input.DOB})
        let response = await studentLogin(input.dptName.value,{id:input.id,DOB:input.DOB})
          localStorage.setItem('jwt',response.token)
          localStorage.setItem("role",response.role)
          localStorage.setItem("route",response.route)
