...
const TeacherDashBoard = () => {
  ...
  useEffect( ()=>{
   ...
   async function getStaffData () {
     setIsLoading(true)
      let response = await getSingleStaff(localStorage.getItem('route'))
  ...
  const handleDeleteStudent = async () =>{
     ...
     if( response.status === 200){
      successNotify("Student successfully delted")
      setDelete([])
     }
      ...
      <section className="relative bg-slate-200 ">
        <header className=" fixed w-full h-[10vh] p-3 flex items-center text-white bg-slate-900 z-20">
          ...
          <nav className="w-[70%] hidden lg:block">
            <ul className="flex justify-evenly">
              ...
              <li onClick={()=>handleOperations("anoancement")}  className={`${operation.anoancement?"border-b-2  border-green-400 0":""} cursor-pointer pb-3 transition-all duration-300`}>Announcement</li>
        ...
        <nav className=" relative w-full flex flex-col  h-[70vh] bg-slate-900 rounded-2xl">
            ...
            <ul className="flex flex-col mt-20 justify-around  h-[40%] ">
              ...
              <li onClick={()=>[handleOperations("anoancement"),handleShowMenu()]}  className={`  bg-green-400 text-white text-center w-[80%] p-1 text-[15px] mx-auto rounded-2xl cursor-pointer transition-all duration-300`}>Announcement</li>
        ...
        <main>
        {
           operation.dashboard && !isLoading ? <div className=" bg-white shadow-lg rounded-3xl p-2 fixed top-[15%] lg:left-[20%] left-[5%] w-[90%] lg:w-[60%] h-[45vh]">
                <div className=" rounded-full w-24 absolute top-8 right-5 h-24">
                   <img src={userData.gender === "male" || "Male" ? maleAvatar : femaleAvatar} alt="avatar" className="rounded-full" />
                </div>
            ...
            </div> : <div className={` ${!isLoading?"hidden":""} space-y-4 animate-pulse bg-gray-400 shadow-lg rounded-3xl p-2 fixed top-[15%] lg:left-[20%] left-[5%] w-[90%] lg:w-[60%] h-[45vh]`}>
                <div className="bg-gray-200 rounded-full w-24 absolute top-8 right-5 h-24">
                 
                </div>
                <div className=" hidden lg:block absolute  right-5 top-[55%]">
                  <p className="bg-slate-300 w-[40%] h-[10%] rounded-2xl"></p>
                </div>
                <h1 className=" font-medium  mt-10 text-3xl bg-slate-300 w-[40%] h-[10%] rounded-2xl"></h1>
                <h3 className="text-[18px] mt-15  font-medium bg-slate-300 w-[40%] h-[10%] rounded-2xl "></h3>
                <h3 className="text-[18px] mt-15  font-medium bg-slate-300 w-[40%] h-[10%] rounded-2xl "></h3>
            </div>
        } 
        </main>
            ...
            studentManage.editStudent && <>
                  {
                    editForm  && <div className="fixed w-[90%] left-0 lg:left-[50%] lg:w-[50%] top-[20%]">
                     ...
                     { sdata.length  ? <ul className="h-[40vh]  w-full lg:w-[50%] flex flex-col justify-evenly items-center lg:m-4 mt-5 lg:mt-10">
                        ...
                        <li className="flex justify-evenly w-full lg:w-[90%] bg-white py-1 px-5 rounded-[10px] shadow-md">
                          <p className=" w-[50%]">Depatment</p>
                          <p className=" w-[50%] ">: {sdata[0].department}</p>
                        </li>
                        <li className="flex justify-evenly  w-full lg:w-[90%] bg-white py-1 px-5 rounded-[10px] shadow-md">
      ...
      }
      {
        ...
        { userData.role === "HOD" ? <aside className=" fixed  lg:hidden top-[12%] left-[1%] w-[98%]  ">
          ...
          { userData.role === 'HOD' ? <div className=" relative mt-[18%] sm:mt-[10%] lg:mt-0 w-[100vw] lg:w-[75vw]">
           ...
           </div> : <div className="  relative mt-[2%]   w-[100vw] lg:w-[75vw] mx-auto "> <Announcement/> </div>
          }
        </div>
      }
      </section>
