...
const StudentInputs = () => {
  ...
  const handleAddStudents = async (data) =>{
    let response = await createStudent(route,data)
    if( response.status === 201){
      notify("Student successfully created")
      reset()
    }
  ...
  return (
    ...
    <section className={` mt-15 lg:mt-0 z-0  h-screen py-15`}>
      ...
      <main className=" flex justify-center lg:justify-end  bg-slate-200 ">
        <form className="  flex flex-col  justify-center w-[95%]   py-5 lg:w-[50%] " onSubmit={handleSubmit(handleAddStudents)} >
